...
// Code Here
const first = (arr, callBack) => {return callBack(arr[0])}
...
//Code Here
const contains = (arr, name, callBack) => arr.includes(name) ? callBack(true) : callBack(false)
// Do not edit the code below.
// Your function is called here
...
//Code Here
const each = (arr, callBack) => arr.forEach((element, idx) => callBack(element, idx))
// Do not edit the code below.
// Your function is called here
...
const evens = (arr) => arr.filter((element) => element % 2 === 0 ? true : false)
...
const startWithLetterA = (arr) => arr.filter((element) => element[0] === "a" || element[0] === "A")
...
const formalGreeting = (arr) => arr.map((element) => element = `Hello, ${element}`)
...
const findProgrammer = (employees) => employees.filter((person) => person.job === "programmer")[0]
...
let orderTotals = orders.map((price) => price = price.price * (price.tax + 1))
...
productOfArray = (arr) => arr.reduce((runningTotal, currentValue) => runningTotal *= currentValue)